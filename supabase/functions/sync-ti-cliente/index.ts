...
// Extrai do texto sequencias que parecem numero de processo/protocolo e devolve
// so os digitos (normalizado), p/ casar com numero_req_normalizado /
// numero_proc_normalizado dos processos. Ex.: "Protocolo (GET): 1593268866" ->
// "1593268866"; "Processo Administrativo no: 44233.539908/2026-98" ->
// "44233539908202698". Threshold de 8 digitos evita ruido; o match e sempre por
// igualdade exata com um numero ja cadastrado, entao falso-positivo e improvavel.
...
serve(async (req) => {
  ...
  if (casoId) {
    ...
    // Mapas numero_normalizado -> id, p/ auto-vincular pela mencao no texto.
    ...
    const judPorNumero = new Map<string, string>();
    {
      ...
      const { data: juds, error: judErr } = await supabase
        .from("processos_judiciais")
        .select("id, numero_proc_normalizado")
        .eq("caso_id", casoId);
      if (judErr) console.error("erro listar processos_judiciais", judErr);
      for (const j of juds || []) {
    ...
    // Dado o texto da nota, tenta achar um processo cadastrado cujo numero seja
    // mencionado. Judicial tem prioridade; senao admin. Fallback: nenhum.
    function resolverProcesso(
      texto: string,
    ): { adminId: string | null; judId: string | null } {
    ...
    for (const nota of notas) {
      ...
      // Tenta vincular pelo numero mencionado no texto da nota.
      const resolved = resolverProcesso(nota.content || "");
      // Vinculo final: match exato por numero tem prioridade; senao cai no
      // processo admin mais antigo (comportamento anterior), ou nada.
      const vincAdminId = resolved.judId
        ? null
        : (resolved.adminId ?? autoProcessoAdminId);
      const vincJudId = resolved.judId;
