...
export function categoriaEtiqueta(nome: string): CategoriaEtiqueta {
  ...
  if (
    /CONCEDIDO|INDEFERIDO|^IMPLANTADO$|JULGADO_|_PROVIDO$|_DESPROVIDO$|_IMPROCEDENTE$|_PROCEDENTE$/.test(
      n,
    )
  ) {
    return 5;
  ...
  // 3) Benefício
  if (
    /^AUXILIO_|^APOSENTADORIA_|^BPC_LOAS|PENSAO_POR_MORTE|SALARIO_MATERNIDADE|^INVALIDEZ|BENEFICIO_POR_INCAPACIDADE|^ADICIONAL_25|^PEDIDO_(DE_PRORROGACAO|FUTURO)|ISENCAO_IMPOSTO_RENDA|^REVISAO_/.test(
      n,
    )
  ) {
    return 3;
  ...
  // 4) Situação processual
  if (
    /^AGUARDANDO_|^AG\.|^ANALISE|^ANALISADO|^MONTAGEM_|^PROTOCOLO_|^RECURSO_|PERICIA|EXIGENCIA|CUMPRIMENTO_SENTENCA|^JUDICIAL$|ADMINISTRATIVO_INSS|LEGAL_MAIL|PLANEJAMENTO_PREVIDENCIARIO|CALCULO_PREVIDENCIARIO|RECEBIDO_CALCULO|MENSAL_PAGANDO|ORGANIZACAO_DOCUMENTOS|PPP_CORRECOES|RECONHECIMENTO_TEMPO_ESPECIAL|REANALISE_ESPECIAL|ACERTOS_DE_VINCULOS|OBRIGACAO_DE_FAZER|PROPOSTA_ACORDO|AVERBACAO|SEM_PROCESSO/.test(
      n,
    )
  ) {
    return 4;
