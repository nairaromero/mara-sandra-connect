...
- Merge `staging → main` é **squash** (um commit só, revert = `git revert <sha>`); logo depois, realinhar a staging por **merge, sem reescrever nada**: `git checkout staging && git pull && git merge origin/main && git push`. Efeito colateral aceito: o PR `staging → main` seguinte lista também os commits antigos (a árvore é a mesma; só o diff importa).
